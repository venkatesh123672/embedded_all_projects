...
extern uint8_t bpon[BUF_LEN_SMALL];
...
//uint32_t data_flash_relay=RELAY_DATA_FLASH_START;




config_data_t configData[10] =
{
		{"<VER>",		"</VER>", 		"TEST",									&SW_ver[0]	},
		{"<WURL>",		"</WURL>",		"http://admin.evegah.com/api/",	    &webURL[0]	},
		{"<APN>",		"</APN>",		"airtelgrps.com",						&APN_URL[0]	},
		{"<VERSION>",	"</VERSION>",	"1.0.0",								&SlNo[0]	},
		{"<DEVID>",		"</DEVID>",		"&dId=EVM1024019",					    &devID[0]	},
		{"<DOM>",		"</DOM>",		"1/1/2023",								dom			},
		{"<BPON>",		"</BPON>",		"oFF",									bpon		},
		{"<BPOFF>",		"</BPOFF>",		"500",									bpoff		},
		{"<TURL>",		"</TURL>",		"www.edel-iot.com/eVega/edel.txt", 		testURL_2	},
		{"<BATTV>",      "</BATTV>",     "52",                                  &BATTERY    }
};
	...
	while(configMode)
	{
    	while(expDataIn)
    	{
    	   	...
    	   	if(newWord_2)
			{
				...
				memset(UART2_RX_BUF,0,strlen(UART2_RX_BUF));
				count_2 = 0;
...
void relay_fmc_write_status(uint8_t relay_status)
{
	 WDT_RESET_COUNTER();
	...
	relay_data=	FMC_Erase(RELAY_DATA_FLASH_START);

	FMC_Write(RELAY_DATA_FLASH_START, relay_status);
...
uint8_t relay_fmc_read_status()
{
	...
	FMC->ISPCTL |=  FMC_ISPCTL_ISPEN_Msk | FMC_ISPCTL_APUEN_Msk | FMC_ISPCTL_LDUEN_Msk | FMC_ISPCTL_CFGUEN_Msk;
     ...
     relay_data=FMC_Read(RELAY_DATA_FLASH_START);
